...
print("Loading data")
data = np.load("mnist_sequence1_sample_8distortions_9x9.npz")
dim = int(math.sqrt(data['X_train'].shape[1]))
x_train, y_train = data['X_train'].reshape((-1, dim, dim)), data['y_train'].squeeze()
x_valid, y_valid = data['X_valid'].reshape((-1, dim, dim)), data['y_valid'].squeeze()
x_test, y_test = data['X_test'].reshape((-1, dim, dim)), data['y_test'].squeeze()
...
l_dense_loc = mnist_cnn.model(l_pool0_loc, input_dim=dim, p=sh_drp, num_classes=0)
...
f_eval = theano.function([x],[output_eval, l_A_eval])
...
for epoch in range(NUM_EPOCH):
    ...
    np.savez( "res_test_2",probs=probs_test, preds=probs_test.argmax(-1),
