...
parser.add_argument("--nb_digits", type=int, default="3", help="nb digits in the images")
parser.add_argument("--output_dim", type=int, default="100", help="output image dimension")
args = parser.parse_args()

ORG_SHP = [28, 28]
OUT_SHP = [args.output_dim, args.output_dim]
NUM_DISTORTIONS = 8
...
all_digits = np.concatenate([X_train,X_val], axis=0)
...
def create_dataset(n, X, labels, org_shp, out_shp):
    ...
    for i in range(n):
        if (i+1) % 1000 == 0:
            print(i)
        x_, y_ = sample_digits(args.nb_digits, X, labels, org_shp)
...
samples = [create_sample(sample_digits(args.nb_digits, X_train, y_train,
...
N_VALID = 10000
N_TEST = 10000
X_train, y_train = create_dataset(N_TRAIN, X_train,y_train, ORG_SHP, OUT_SHP)
X_valid, y_valid = create_dataset(N_VALID, X_val,y_val, ORG_SHP, OUT_SHP)
X_test, y_test = create_dataset(N_TEST, X_test, y_test, ORG_SHP, OUT_SHP)
np.savez_compressed(
    "mnist_sequence" + str(args.nb_digits) + "_sample_8distortions_9x9",
    X_train=X_train,
    y_train=y_train,
    X_valid=X_valid,
    y_valid=y_valid,
